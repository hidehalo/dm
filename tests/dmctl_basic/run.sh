...
function run() {
    ...
    gsed -i "s/password: \"\"/password: \"wrond password\"/g" $WORK_DIR/dm-task-error-database-config.yaml
...
#    show_ddl_locks_no_locks $TASK_NAME
    ...
    run_sql "show binary logs\G" $MYSQL_HOST1 $MYSQL_PORT1 $MYSQL_PASSWORD1
