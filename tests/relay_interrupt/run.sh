...
function prepare_data1() {
    run_sql 'DROP DATABASE if exists relay_interrupt;' $MYSQL_HOST1 $MYSQL_PORT1 $MYSQL_PASSWORD1
    run_sql 'CREATE DATABASE relay_interrupt;' $MYSQL_HOST1 $MYSQL_PORT1 $MYSQL_PASSWORD1
    run_sql "CREATE TABLE relay_interrupt.t$1(i TINYINT, j INT UNIQUE KEY);" $MYSQL_PORT1 $MYSQL_PASSWORD1
    for j in $(seq 100); do
        run_sql "INSERT INTO relay_interrupt.t$1 VALUES ($j,${j}000$j),($j,${j}001$j);" $MYSQL_HOST1 $MYSQL_PORT1 $MYSQL_PASSWORD1
    done
}

function prepare_data2() {
    run_sql "DELETE FROM relay_interrupt.t$1 limit 1;" $MYSQL_HOST1 $MYSQL_PORT1 $MYSQL_PASSWORD1
}
...
function run() {
    ...
    for(( i=0;i<${#failpoints[@]};i++)) do
        ...
        run_sql 'DROP DATABASE if exists dm_meta;' $MYSQL_HOST1 $TIDB_PORT $TIDB_PASSWORD
        run_sql 'DROP DATABASE if exists relay_interrupt;' 127.0.0.1 $TIDB_PORT $TIDB_PASSWORD
        prepare_data1 $i
